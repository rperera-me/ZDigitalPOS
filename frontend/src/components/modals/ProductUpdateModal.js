...
export default function ProductUpdateModal({ 
  ...
  // State for batch price editing (Unit products)
  const [editingBatchId, setEditingBatchId] = useState(null);
  const [editSellingPrice, setEditSellingPrice] = useState("");
  const [editWholesalePrice, setEditWholesalePrice] = useState("");

  // State for Kg product pricing (global, not per-batch)
  const [editKgProductPrice, setEditKgProductPrice] = useState("");
  const [editKgSellingPrice, setEditKgSellingPrice] = useState("");
  const [editKgWholesalePrice, setEditKgWholesalePrice] = useState("");

  const isKgProduct = product?.measureType === "Kg";
  ...
  const fetchBatches = async () => {
    ...
    try {
      ...
      setBatches(data);
      if (product.measureType === "Kg" && data.length > 0) {
        setEditKgProductPrice(data[0].productPrice.toString());
        setEditKgSellingPrice(data[0].sellingPrice.toString());
        setEditKgWholesalePrice(data[0].wholesalePrice.toString());
      }
    } catch (error) {
  ...
  const handleSaveKgPrices = async () => {
    if (!editKgSellingPrice || !editKgWholesalePrice) {
      alert("Please enter selling and wholesale prices");
      return;
    }
    if (parseFloat(editKgSellingPrice) <= 0 || parseFloat(editKgWholesalePrice) <= 0) {
      alert("Prices must be greater than 0");
      return;
    }
    ...
    try {
      ...
      await Promise.all(batches.map(batch =>
        api.put(`/product/batches/${batch.id}`, {
          ...batch,
          productPrice: parseFloat(editKgProductPrice) || batch.productPrice,
          sellingPrice: parseFloat(editKgSellingPrice),
          wholesalePrice: parseFloat(editKgWholesalePrice),
        })
      ));
      alert("Kg prices updated successfully!");
      fetchBatches();
      onProductUpdated();
    } catch (error) {
      console.error("Failed to update Kg prices:", error);
  ...
  const resetForm = () => {
    ...
    setEditSellingPrice("");
    setEditWholesalePrice("");
    setEditKgProductPrice("");
    setEditKgSellingPrice("");
    setEditKgWholesalePrice("");
  };
          ...
          {isKgProduct ? (
            <>
              <div className="flex justify-between items-center mb-4">
                <h4 className="font-semibold text-lg text-orange-900 flex items-center gap-2">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mb-4">
                <p className="text-sm text-orange-800">
                  Pricing for Kg products applies globally — all stock batches will be updated with the same prices.
                </p>
              </div>
              ...
              {loadingBatches ? (
                ...
                <div className="text-center py-8">
                  <p className="text-gray-500">No stock yet. Add stock through GRN to set pricing.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        Kg Price (MRP) <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="number"
                        step="0.01"
                        min="0"
                        value={editKgProductPrice}
                        onChange={(e) => setEditKgProductPrice(e.target.value)}
                        onWheel={(e) => e.target.blur()}
                        className="w-full border-2 border-yellow-400 rounded-lg p-2 focus:outline-none focus:border-yellow-600 bg-yellow-50"
                        placeholder="Kg Price (MRP)"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                      ...
                      <input
                        ...
                        step="0.01"
                        min="0"
                        value={editKgSellingPrice}
                        onChange={(e) => setEditKgSellingPrice(e.target.value)}
                        onWheel={(e) => e.target.blur()}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        Wholesale Price / kg <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="number"
                        step="0.01"
                        min="0"
                        value={editKgWholesalePrice}
                        onChange={(e) => setEditKgWholesalePrice(e.target.value)}
                        onWheel={(e) => e.target.blur()}
                  ...
                  <button
                    onClick={handleSaveKgPrices}
                    className="w-full bg-orange-600 text-white py-3 rounded-lg hover:bg-orange-700 transition font-semibold flex items-center justify-center gap-2"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                    </svg>
            ...
            <>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                <p className="text-sm text-blue-800">
                  ...
                  Unit Price (MRP) and Cost Price are managed through GRN entries.
                </p>
              </div>
                        ...
                        {editingBatchId === batch.id ? (
                          ...
                          <div className="grid grid-cols-4 gap-4">
                            ...
                            <div className="bg-yellow-50 p-3 rounded-lg border border-yellow-200">
                              <div className="text-xs text-gray-600 mb-1">Unit Price (MRP)</div>
